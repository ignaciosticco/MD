...
double rin = 1.8;  //  r de corte interno
double rout = 3.0; //  r de corte interno (V=0 si r>r0ut)
double sigma = 1.0;
...
int tabla_fuerzas(int nf, double *fuerzas){
	...
	for (int i=0; i < nf; i++){
			...
			if (r <= rin) {
				...
				nuevo_numerador2 = (rout*rout-r*r)*(rout*rout-r*r)*4.0*r;
				nuevo_denominador = (rout*rout - rin*rin)*(rout*rout - rin*rin)*(rout*rout - rin*rin);
				...
				s = (numerador1*numerador2)/denominador;
			       	...
			       	fuerzas[i] = nuevo_s*4.0*(pow(sigma/r,12.)-pow(sigma/r,6.))+4.0*s*12.0*(pow(sigma/r,12.)-0.5*pow(sigma/r,6.))/r;
			}
			else {
				fuerzas[i] = 0.0;	
			}
